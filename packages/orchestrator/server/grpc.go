...
func (s *server) Create(ctx context.Context, req *orchestrator.SandboxCreateRequest) (*orchestrator.SandboxCreateResponse, error) {
	...
	return &orchestrator.SandboxCreateResponse{
		Info: sbxInfo.ToProtoSandboxInfo(),
	}, nil
}
...
func (s *server) List(ctx context.Context, req *orchestrator.SandboxListRequest) (*orchestrator.SandboxListResponse, error) {
	...
	if req.Orphan {
		sandboxes, err = s.listOrphan(childCtx)
	} else {
		sandboxes, err = s.list(childCtx, req.Running)
	}
	if err != nil {
	...
	for _, sbx := range sandboxes {
		result.Sandboxes = append(result.Sandboxes, sbx.ToProtoSandboxInfo())
	}
	return &result, nil
}
...
func (s *server) getSandboxInfoFromProc(ctx context.Context, proc *process.Process) (sandbox.SandboxInfo, error) {
	var info sandbox.SandboxInfo
	cmdline, err := proc.Cmdline()
	if err != nil {
		return info, SandboxNotFound
	}
	...
	if err != nil {
		...
		return info, fmt.Errorf("find process but cannot search sandbox network: %w", err)
	}
	templateID, err := parseTemplateIdFromOrphanProcess(proc)
	if err != nil {
		telemetry.ReportCriticalError(ctx, err)
		return info, fmt.Errorf("find process but cannot parse template id: %w", err)
	}
	// for orphan sandbox, we only populate privateIP and sandboxID
	// NOTE: maybe we can return pid to reduce the overhead for
...
func (s *server) listOrphan(ctx context.Context) ([]sandbox.SandboxInfo, error) {
	processes, err := process.Processes()
	if err != nil {
		return nil, fmt.Errorf("cannot get processes on orchestrator: %w", err)
	}
	var results []sandbox.SandboxInfo
	for _, process := range processes {
		...
		info, err := s.getSandboxInfoFromProc(ctx, process)
		if err != nil {
			if errors.Is(err, SandboxNotFound) {
				continue
			}
			telemetry.ReportCriticalError(ctx, err, attribute.String("cmdline", cmdline))
		}
...
func (s *server) list(_ context.Context, running bool) ([]sandbox.SandboxInfo, error) {
	s.mu.Lock()
	results := make([]sandbox.SandboxInfo, 0, len(s.sandboxes))
...
func (s *server) Search(ctx context.Context, req *orchestrator.SandboxSearchRequest) (*orchestrator.SandboxSearchResponse, error) {
	...
	sbxInfo := sbx.GetSandboxInfo()
	return &orchestrator.SandboxSearchResponse{
		Sandbox: sbxInfo.ToProtoSandboxInfo(),
	}, nil
}
...
func (s *server) Purge(ctx context.Context, req *orchestrator.SandboxPurgeRequest) (*empty.Empty, error) {
	...
	if req.PurgeAll {
		...
		for _, sandboxID := range req.SandboxIDs {
			...
			info, err := s.getSandboxInfoFromProc(ctx, process)
			if err != nil {
				msg := fmt.Sprintf("get sandbox info failed: %s", err.Error())
...
func (s *server) CleanNetworkEnv(ctx context.Context, req *orchestrator.HostManageCleanNetworkEnvRequest) (*empty.Empty, error) {
	...
	for _, networkIdx := range req.GetNetworkIDs() {
		...
		s.netManager.DNS().RemoveAddress(net.HostClonedIP().String())
	}
	if finalErr != nil {
