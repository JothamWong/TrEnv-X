...
func (net *SandboxNetworkWrapper) Cleanup(ctx context.Context) error {
	...
	for _, c := range caps {
		defer unix.Prctl(unix.PR_CAP_AMBIENT, unix.PR_CAP_AMBIENT_LOWER, c, 0, 0)
	}

	return net.SandboxNetwork.Cleanup(ctx)
}
...
func (m *NetworkManager) GetSandboxNetwork(
	...
	if len(m.free) > 0 {
		...
		net, err := newSandboxNetwork(childCtx, tracer, idx, subnet)
		if err != nil {
			return nil, err
		}
	...
	if err = m.CreateDNSEntry(wrapper.HostClonedIP().String(), sandboxID); err != nil {
