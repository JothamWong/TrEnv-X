...
// SandboxInfo used internally (different from protobuf definition).
type SandboxInfo struct {
	SandboxID          string
	TemplateID         string
	Pid                int
	KernelVersion      string
	NetEnv             network.NetworkEnv
	EnableDiffSnapshot bool
	StartTime          time.Time
	State              orchestrator.SandboxState
}

func (s *Sandbox) GetSandboxInfo() SandboxInfo {
	// This is a read only function. Thus, we do not get lock here.
	// Or else, it might conflict with other function (e.g., cleanup).
	return SandboxInfo{
		SandboxID:          s.SandboxID(),
		Pid:                s.vmm.cmd.Process.Pid,
		TemplateID:         s.Config.TemplateID,
		KernelVersion:      s.Config.KernelVersion,
		NetEnv:             s.Net.NetworkEnv,
		EnableDiffSnapshot: s.Config.EnableDiffSnapshot,
		StartTime:          s.StartAt,
		State:              s.State,
	}
}

func (info *SandboxInfo) ToProtoSandboxInfo() *orchestrator.SandboxInfo {
	pid := uint32(info.Pid)
	netIdx := int64(info.NetEnv.NetworkIdx())
	ip := info.NetEnv.HostClonedIP().String()
	return &orchestrator.SandboxInfo{
		...
		NetworkIdx:          &netIdx,
		PrivateIP:           &ip,
		EnableDiffSnapshots: &info.EnableDiffSnapshot,
