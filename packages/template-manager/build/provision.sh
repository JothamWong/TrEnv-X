...
DEBIAN_FRONTEND=noninteractive DEBCONF_NOWARNINGS=yes apt-get install -y \
  openssh-server sudo systemd socat chrony linuxptp lsof vim less
# xvfb x11vnc
...
mkdir -p {{ .StartCmdWorkingDirectory }}
chown -R user:user {{ .StartCmdWorkingDirectory }}
{{ end -}}

# Add start command service if the start command is not empty.
# note that we use 'EOF' to prevent vairable expansion in the heredoc.
{{ if .StartCmd -}}
