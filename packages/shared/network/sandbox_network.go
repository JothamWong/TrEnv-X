...
func (n *SandboxNetwork) SetupIptablesAndRoute() error {
	...
	if err := utils.RaiseAmbientCaps(caps); err != nil {
		return err
	}
	for _, c := range caps {
		n.end = append(n.end, func() error {
			return unix.Prctl(unix.PR_CAP_AMBIENT, unix.PR_CAP_AMBIENT_LOWER, c, 0, 0)
		})
	}
	...
	err = tables.Append("nat", "POSTROUTING", "-o", n.VpeerName(),
		"-s", n.GuestIP(), "-j", "SNAT",
		"--to-source", n.HostClonedIP().String(),
	)
	if err != nil {
	...
	err = tables.Append("nat", "PREROUTING", "-i", n.VpeerName(),
		"-d", n.HostClonedIP().String(), "-j", "DNAT",
	...
	hostClonedCIDR := n.HostClonedCIDR()
	err = netlink.RouteAdd(&netlink.Route{
		// Gw means next hop
		Gw:  n.VpeerIP(),
		Dst: &hostClonedCIDR,
	...
	err = tables.Append("nat", "POSTROUTING", "-s", n.HostClonedIP().String(), "-o", hostDefaultGateway, "-j", "MASQUERADE")
	if err != nil {
...
func (n *SandboxNetwork) DeleteHostIptables() (finalErr error) {
	...
	err = tables.Delete("nat", "POSTROUTING", "-s", n.HostClonedIP().String(), "-o", hostDefaultGateway, "-j", "MASQUERADE")
	if err != nil {
